                ...
                for i, task in tasks_df.iterrows():
                    ...
                    try:
                        ...
                        if best_machine_row is not None:
                            ...
                            # Debug: Print scores to check if they're being calculated
                            ...
                            result = {
                                ...
                                "energy_score": f"{energy_score:.3f}" if energy_score is not None else "0.000",
                                "fit_score": f"{fit_score:.3f}" if fit_score is not None else "0.000"
                            }
                        else:
                            result = {
                                "task_id": i+1,
...
if st.session_state.simulation_results:
    ...
    with tab2:
        if st.session_state.machines:
            # Machine utilization overview - show ALL historical data
            machine_util_data = []
            
            # Collect all utilization history from all machines
            for machine in st.session_state.machines:
                for hist_point in machine.utilization_history:
                    machine_util_data.append({
                        'Machine': machine.name,
                        'Time': hist_point['time'],
                        'CPU Utilization': hist_point['cpu_util'] * 100,
                        'GPU Utilization': hist_point['gpu_util'] * 100,
                        'RAM Utilization': hist_point['ram_util'] * 100,
                        'Active Tasks': hist_point['active_tasks']
                    })
            ...
            if machine_util_data:
                ...
                # Current utilization snapshot (latest values only)
                latest_util_data = []
                for machine in st.session_state.machines:
